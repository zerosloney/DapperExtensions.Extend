...
namespace DapperExtensions.Extend
{
    internal static class PredicateBuilder
    {
        ...
        {
            if (ExpressionUtils.IsConstant(first, true))
                return second;
            if (ExpressionUtils.IsConstant(second, true))
                return first;
            if (ExpressionUtils.IsConstant(first, false) || ExpressionUtils.IsConstant(second, false))
                return False<T>();
        ...
        public static Expression<Func<T, bool>> Or<T>(this Expression<Func<T, bool>> first,
            Expression<Func<T, bool>> second)
        {
            if (ExpressionUtils.IsConstant(first, false))
                return second;
            if (ExpressionUtils.IsConstant(second, false))
                return first;
            if (ExpressionUtils.IsConstant(first, true) || ExpressionUtils.IsConstant(second, true))
                return True<T>();
            return first.Combine(second, (a, b) => a || b);
        }
